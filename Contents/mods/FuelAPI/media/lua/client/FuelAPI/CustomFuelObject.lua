...
local function onPreFillWorldObjectContextMenu(player, context, worldobjects, test)
    ...
    if customFuelObject then
		...
		if FittingFuelTypes == nil then --- in case barrel fuel type was not initialized
			FittingFuelTypes = FuelTypesTableRS["Gasoline"]
			customFuelObject.isoObject:getModData().RSFuelType = "Gasoline"
			customFuelObject.isoObject:transmitModData();
		end 
